...
const server=http.createServer(async(req,res)=>{try{
  ...
  if(req.method==='GET'&&url.pathname==='/api/health')return json(res,200,{ok:true,version:'1.7.0',architecture:'gemini-background-polling',judge_configured:Boolean(cfg.geminiApiKey),judge_model:cfg.geminiModel,api_token_configured:Boolean(cfg.pqaApiToken),analysis_protected:Boolean(cfg.protectAnalysis)});
...
server.listen(cfg.port,'0.0.0.0',()=>console.log(`Prompt Quality Analyzer v1.7.0 listening on http://0.0.0.0:${cfg.port} (${cfg.geminiApiKey?'Gemini background judge enabled':'deterministic-only'})`));

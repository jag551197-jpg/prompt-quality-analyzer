...
const DANGER_CODES=new Set(['forced-certainty','infer-missing','time-sensitive-without-grounding','unsafe-destructive-action','causal-overclaim','conflict-resolution-by-guessing','unbounded-tool-use','conflicting-constraints','uncertainty-suppression','citation-fabrication']);
const REASON_LABELS={
  'forced-certainty':'FORCED_CERTAINTY','infer-missing':'UNSUPPORTED_INFERENCE','time-sensitive-without-grounding':'CURRENT_FACTS_UNGROUNDED',
  ...
  'citation-fabrication':'CITATION_FABRICATION','evaluator-manipulation':'EVALUATOR_MANIPULATION','system-prompt-extraction':'SYSTEM_PROMPT_EXTRACTION','role-override':'INSTRUCTION_HIERARCHY_ATTACK','weak-grounding':'WEAK_GROUNDING','no-abstention':'NO_ABSTENTION','no-citations':'NO_CITATIONS'
};
function confidenceFor(staticAnalysis,judgeConfidence=null){
  const det=Math.min(.98,.72+(staticAnalysis.protective_controls?.length||0)*.035+(staticAnalysis.explicit_danger_count||0)*.05);
  return Number((Number.isFinite(Number(judgeConfidence)) ? det*.35+Number(judgeConfidence)*.65 : det).toFixed(2));
}
function scoreRange(score,confidence){const half=Math.max(2,Math.round((1-confidence)*18));return [clamp(score-half),clamp(score+half)];}
function reasonCodes(staticAnalysis){return [...new Set(staticAnalysis.risk_indicators||[])].map(x=>REASON_LABELS[x]||x.toUpperCase().replace(/-/g,'_'));}
...
export function buildDeterministicResult(input){
  ...
  return {version:'1.7.0',rubric_version:RUBRIC_VERSION,scoring_profile:profileFor(input.intendedUse),mode:'deterministic-only',timing:{total_ms:nowMs(start),deterministic_ms:nowMs(start),gemini_ms:null},judge:{provider:null,model:null,status:'not-run',confidence:null,error:null,error_category:null,http_status:null,duration_ms:null},judge_response:null,overall_score:overall,quality_level:qualityLevel(overall),hallucination_risk:staticAnalysis.hallucination_risk,risk_confidence:confidenceFor(staticAnalysis),score_confidence:confidenceFor(staticAnalysis),score_range:scoreRange(overall,confidenceFor(staticAnalysis)),evaluation_mode:staticAnalysis.evaluation_mode,reason_codes:reasonCodes(staticAnalysis),dimensions:dimensionsFrom(scores,input.intendedUse),strengths:staticStrengths(staticAnalysis),weaknesses:staticAnalysis.issues.slice(0,10),risk_indicators:staticAnalysis.risk_indicators.slice(0,10),recommendations:staticAnalysis.recommendations.slice(0,10),improved_prompt:null,deterministic:staticAnalysis,calibration:{rubric_version:RUBRIC_VERSION,profile:profileFor(input.intendedUse),risk_model:'evidence-tiered-v3'},disclaimer:'Hallucination risk identifies prompt-level risk factors; it does not predict with certainty whether a model will hallucinate.'};
}
...
function fuseRisk(staticAnalysis,judgeRisk){
  ...
  const currentMitigated=Boolean(staticAnalysis?.flags?.current&&staticAnalysis?.flags?.currentRetrieval&&(staticAnalysis?.flags?.hasGround||staticAnalysis?.flags?.hasCitation));
  if(explicit>0)return'high';
  if(currentMitigated&&protections>=2&&judgeRisk!=='high')return'low';
  if(judgeRisk==='high'&&contextual>=2&&protections===0)return'high';
  if(judgeRisk==='high'&&protections>=2)return'medium';
  if(judgeRisk==='low'&&protections>=1)return'low';
  if(staticAnalysis?.hallucination_risk==='low'&&judgeRisk!=='high')return'low';
  if(judgeRisk==='medium'&&protections>=2&&contextual<=1)return'low';

...
export function deterministicAnalyze({prompt,context='',intendedUse='general',requiresCurrentFacts=false,evaluationMode='auto'}){
  ...
  const explicitDanger=signals.filter(s=>['forced-certainty','infer-missing','time-sensitive-without-grounding','unsafe-destructive-action','causal-overclaim','conflict-resolution-by-guessing','unbounded-tool-use','conflicting-constraints','uncertainty-suppression','citation-fabrication'].includes(s));
  ...
  return {profile,evaluation_mode:resolvedEvaluationMode,metrics:{prompt_chars:p.length,context_chars:context.length,duplicate_ratio:Number(dup.toFixed(3))},flags:{current,hasGround,hasAbstain:hasAbstain||noGuess,hasCitation,hasFormat,hasTool,boundedTool,forced,inferMissing,noGuess,destructive,safeDestructive,causalOverclaim,causalSafe,nullMissing,conflictSafe,conflictBad,repeatedTool,endlessTool,impossibleConstraint,intentionalRepeat,uncertaintySuppression,fabricateCitation,currentRetrieval,templateDetected},issues:[...new Set(issues)],recommendations:[...new Set(recommendations)],risk_indicators:[...new Set(signals)],protective_controls:[...new Set(protective)],explicit_danger_count:explicitDanger.length,contextual_risk_count:contextualRisk.length,hallucination_risk:risk};
}

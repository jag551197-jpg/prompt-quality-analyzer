const RX = {
  ...
  conflictSafe:/\b(report|surface|show|state|identify|highlight).{0,35}(the )?(conflict|disagreement)|do not reconcile|mark.*ambiguous|all candidates|if.{0,40}conflict.{0,60}(report|surface|show|state|highlight)/i,
  conflictBad:/\b(if|when).{0,30}(sources?|documents?|evidence|values?).{0,30}(conflict|disagree).{0,100}(choose|pick|select|use).{0,60}(likely|plausible|best|most likely)|silently.{0,60}(choose|pick|resolve)|choose.*seems (right|likely|plausible)/i,
  uncertaintySuppression:/\b(hide|mask|suppress|omit|never mention|do not mention|don't mention).{0,40}(uncertainty|limitations?|confidence|caveats?)|sound authoritative.{0,50}(regardless|even if)|never (express|show|state).{0,30}(uncertainty|doubt)/i,
  fabricateCitation:/\b(invent|fabricate|make up|create).{0,35}(citation|source|paper|reference|url)|if.{0,25}(citation|source|url).{0,25}(missing|unknown).{0,35}(guess|invent|infer)/i,
  currentRetrieval:/\b(search|browse|retrieve|look up|query).{0,45}(current|latest|recent|authoritative|official|primary|sources?|reports?)|\b(current|latest|recent).{0,45}(search|browse|retrieve|authoritative|official|primary sources?)/i,
  judgeOverride:/\b(ignore|bypass|override|disregard).{0,50}(evaluator|judge|rubric|scoring|system instructions?)|(?:give|assign|return).{0,30}(100\/100|score of 100|low risk).{0,40}(regardless|no matter|without evaluating)/i,
  systemPromptExtraction:/\b(reveal|print|show|repeat|extract).{0,40}(system prompt|hidden instructions?|evaluator instructions?|rubric)/i,
  roleOverride:/\b(you are now|act as).{0,40}(the evaluator|system|judge)|\bignore previous instructions?\b/i,
  templateToken:/\{\{[^}]{1,80}\}\}|\$\{[^}]{1,80}\}|\[[A-Z][A-Z0-9 _-]{2,60}\]|<([a-z][a-z0-9_-]{2,40})>/i,
};
...
export function deterministicAnalyze({prompt,context='',intendedUse='general',requiresCurrentFacts=false,evaluationMode='auto'}){
  ...
  const templateDetected=RX.templateToken.test(p);
  const resolvedEvaluationMode=evaluationMode==='template'||(evaluationMode==='auto'&&templateDetected)?'template':'instantiated';
  const currentRetrieval=RX.currentRetrieval.test(p);
  ...
  if(current&&!currentRetrieval&&!hasTool&&!hasGround){issues.push('Time-sensitive facts are requested without a retrieval or grounding requirement.');recommendations.push('Require current retrieval/search or provide authoritative current context.');signals.push('time-sensitive-without-grounding');}
  ...
  if(uncertaintySuppression){issues.push('The prompt explicitly suppresses uncertainty, limitations, or confidence disclosure.');recommendations.push('Require uncertainty and limitations to be surfaced when evidence is incomplete.');signals.push('uncertainty-suppression');}
  if(judgeOverride){issues.push('The analyzed prompt attempts to manipulate the evaluator or requested score.');recommendations.push('Remove evaluator-manipulation instructions from the downstream task.');signals.push('evaluator-manipulation');}
  if(systemPromptExtraction){issues.push('The prompt attempts to extract hidden evaluator or system instructions.');recommendations.push('Remove system-prompt extraction instructions.');signals.push('system-prompt-extraction');}
  if(roleOverride){issues.push('The prompt contains instruction-hierarchy or role-override language.');recommendations.push('Keep downstream task instructions separate from evaluator/system controls.');signals.push('role-override');}
  if(fabricateCitation){issues.push('The prompt permits fabricated citations, sources, papers, or URLs.');recommendations.push('Require citations to be verified and report missing sources instead of inventing them.');signals.push('citation-fabrication');}
  ...
  if(hasGround||missingDocSafe)protective.push('grounding'); if(current&&currentRetrieval)protective.push('current-retrieval'); if(resolvedEvaluationMode==='template')protective.push('template-contract'); if(hasAbstain||noGuess)protective.push('abstention'); if(hasCitation)protective.push('citations'); if(hasFormat)protective.push('output-contract'); if(boundedTool)protective.push('bounded-tools'); if(nullMissing)protective.push('null-on-missing'); if(conflictSafe)protective.push('conflict-reporting'); if(safeDestructive)protective.push('destructive-confirmation'); if(causalSafe)protective.push('causal-discipline');
  const explicitDanger=signals.filter(s=>['forced-certainty','infer-missing','time-sensitive-without-grounding','unsafe-destructive-action','causal-overclaim','conflict-resolution-by-guessing','unbounded-tool-use','conflicting-constraints','uncertainty-suppression','citation-fabrication','evaluator-manipulation','system-prompt-extraction','role-override'].includes(s));
  ...
  return {profile,evaluation_mode:resolvedEvaluationMode,metrics:{prompt_chars:p.length,context_chars:context.length,duplicate_ratio:Number(dup.toFixed(3))},flags:{current,hasGround,hasAbstain:hasAbstain||noGuess,hasCitation,hasFormat,hasTool,boundedTool,forced,inferMissing,noGuess,destructive,safeDestructive,causalOverclaim,causalSafe,nullMissing,conflictSafe,conflictBad,repeatedTool,endlessTool,impossibleConstraint,intentionalRepeat,uncertaintySuppression,fabricateCitation,currentRetrieval,templateDetected,judgeOverride,systemPromptExtraction,roleOverride},issues:[...new Set(issues)],recommendations:[...new Set(recommendations)],risk_indicators:[...new Set(signals)],protective_controls:[...new Set(protective)],explicit_danger_count:explicitDanger.length,contextual_risk_count:contextualRisk.length,hallucination_risk:risk};
}

...
TERMINAL={"complete","completed","failed","cancelled","invalid-result","incomplete","budget_exceeded","submit-failed","not-configured"}
...
def flatten(c):
    r=c.get("final_result") or c.get("result") or c.get("analysis") or {}
    if isinstance(r.get("analysis"),dict): r=r["analysis"]
    e=c.get("expectation") or c.get("expectations") or c.get("expectation_result") or {}
    ep=(e.get("pass") if "pass" in e else e.get("passed")) if isinstance(e,dict) else c.get("passed")
    return {"id":c.get("id") or c.get("case_id") or c.get("name") or "unnamed",
            "category":c.get("category","") or c.get("payload",{}).get("intendedUse",""),"status":c.get("status",("completed" if c.get("final_result") else "")),
            ...
            "pass":ep,"prompt":c.get("prompt","") or c.get("payload",{}).get("prompt",""),
            "weaknesses":r.get("weaknesses") or [],"recommendations":r.get("recommendations") or [],
            "improved_prompt":r.get("improved_prompt")}

def integrity_ok(c):
    f=flatten(c)
    return (str(f["status"]).lower()=="completed" and isinstance(f["score"],(int,float)) and f["risk"] in ("low","medium","high") and isinstance((c.get("final_result") or c.get("result") or c.get("analysis") or {}).get("recommendations"),list))
...
def merged(cp):
    ...
    return {"cases":cs,"batches":batches,"summary":{"total_cases":cp["total_cases"],"reported_cases":len(cs),"completed_cases":donec,
            ...
            "average_overall_score":round(sum(scores)/len(scores),2) if scores else None,
            "batch_count":len(cp["batches"]),"completed_batches":sum(1 for b in cp["batches"] if b["status"]=="completed"),"result_integrity_rate":round(donec/max(1,cp["total_cases"])*100,2)}}

def render(cp,src,base):
    m=merged(cp); s=m["summary"]; L=["# Prompt Quality Analyzer Benchmark Report","",f"- **Generated:** {now()}",f"- **Benchmark:** `{src}`",f"- **API:** `{base}`",f"- **Runner:** v3.1 resilient sequential",f"- **Batch size:** {cp['batch_size']}",f"- **Checkpoint:** `{cp['checkpoint_file']}`","","## Executive Summary","","| Metric | Result |","|---|---:|",
        f"| Total cases | {s['total_cases']} |",f"| Cases returned | {s['reported_cases']} |",f"| Completed cases | {s['completed_cases']} |",f"| Completed batches | {s['completed_batches']}/{s['batch_count']} |",f"| Expectation passes | {s['expectation_passes']} |",f"| Expectation failures | {s['expectation_failures']} |",f"| Pass rate | {s['expectation_pass_rate'] if s['expectation_pass_rate'] is not None else 'N/A'}% |",f"| Average score | {s['average_overall_score'] if s['average_overall_score'] is not None else 'N/A'} |",f"| Result integrity rate | {s.get('result_integrity_rate',0)}% |","","## Batch Status","","| Batch | Status | Benchmark ID | Completed | Total |","|---:|---|---|---:|---:|"]
    for b in m["batches"]:
...
def main():
    ...
    for b in cp["batches"]:
        ...
        while True:
            ...
            if done(st):
                bad=[c for c in cases_of(st) if not integrity_ok(c)]
                if bad:
                    b["status"]="invalid_result"
                    b["last_error"]=f"{len(bad)} terminal case(s) failed result-integrity contract"
                    atomic(cpf,cp); write_report(cp,src,base,out)
                    print(f"ERROR batch {i}: {len(bad)} completed case(s) missing final score/risk/result contract",file=sys.stderr)
                    return 3
                b["status"]="completed"; atomic(cpf,cp); write_report(cp,src,base,out); print(f"Batch {i} complete"); break

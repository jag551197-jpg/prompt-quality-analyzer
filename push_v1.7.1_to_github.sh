...
WORK="${PQA_WORK:-$HOME/Downloads/pqa-community-v1.8.2-push}"
...
npm run check
git add -A
if ! git diff --cached --quiet; then git commit -m "fix: PQA Community v1.8.2 CSP-safe multilingual UI"; git push origin HEAD:main; fi
if ! git rev-parse community-v1.8.2 >/dev/null 2>&1; then git tag -a community-v1.8.2 -m "PQA Community v1.8.2 — CSP-safe multilingual corrective release"; git push origin community-v1.8.2; fi
